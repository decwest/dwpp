...
```bash
PYTHONPATH=. UV_CACHE_DIR=/tmp/dwpp-uv-cache uv run python scripts/revision/run_studies.py --study all
```
...
```bash
PYTHONPATH=. UV_CACHE_DIR=/tmp/dwpp-uv-cache uv run python scripts/revision/test_revision.py
```
...
The manuscript studies and their tables use PathC only; the smoke study intentionally retains PathA, PathB, and PathC as an internal validation sweep.
